package ec.cjpq.fe.model;
// Autor: CARLOS JULIO PEREZ QUIZHPE carlosjulioperez@gmail.com 099-3208265 Guayaquil-Ecuador
// Generated 10/07/2016 07:32:40 PM by Hibernate Tools 3.2.4.GA

import javax.persistence.AttributeOverride;
import javax.persistence.AttributeOverrides;
import javax.persistence.Column;
import javax.persistence.EmbeddedId;
import javax.persistence.Entity;
import javax.persistence.Table;

/**
 * Exchangerate por hbm2java el 10/07/2016 07:32:40 PM - Hibernate Tools 3.2.4.GA
 */
@Entity
@Table(name="exchangerate",schema="public")
public class Exchangerate  implements java.io.Serializable
 {


     private ExchangerateId id;

    public Exchangerate() {
    }

    public Exchangerate(ExchangerateId id) {
       this.id = id;
    }
   
     @EmbeddedId

    
    @AttributeOverrides( {
        @AttributeOverride(name="curr", column=@Column(name="curr", length=3) ), 
        @AttributeOverride(name="transdate", column=@Column(name="transdate", length=13) ), 
        @AttributeOverride(name="buy", column=@Column(name="buy", precision=17, scale=17) ), 
        @AttributeOverride(name="sell", column=@Column(name="sell", precision=17, scale=17) ) } )
    public ExchangerateId getId() {
        return this.id;
    }
    
    public void setId(ExchangerateId id) {
        this.id = id;
    }




}


